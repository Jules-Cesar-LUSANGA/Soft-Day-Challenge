...
function isLikedArticle($article_id,$username){
    ...
    $sql = $bdd->prepare("SELECT article_id,username FROM articles_likes WHERE article_id = ? AND username=?");
...
function like_article($article_id){
    ...
    if (isLikedArticle($article_id, $username)==false) {
        ...
        $sql = $bdd->prepare("INSERT INTO articles_likes(article_id,username) VALUES(?,?)");
        $sql->execute(array($article_id,$username));  
...
function dislike_article($article_id){
    ...
    if (isLikedArticle($article_id, $username)==true) {
        ...
        $sql = $bdd->prepare("DELETE FROM articles_likes WHERE article_id=? AND username=?");
        $sql->execute(array($article_id,$username));  
...
function publishArticle($username, $title, $category, $content){
    ...
    $sql = $bdd->prepare("INSERT INTO blog_articles(author,article_title,category, content,article_date) VALUES(?,?,?,?,?)");

    $article_date = Date("Y/m/d H:i:s");

    // Exécution de la requête
    $sql->execute(array($username, $title, $category, $content, $article_date));
}
